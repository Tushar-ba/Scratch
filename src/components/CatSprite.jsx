...
function CatSprite({ actions }) {
  ...
  useEffect(() => {
    const executeActions = () => {
      // Clear previous actions if needed
      setPosition({ x: 0, y: 0 });
      setRotation(0);
      setIsVisible(true);
      setMessage("");
      ...
      actions.forEach((action, index) => {
        setTimeout(() => {
          if (action === "Move 10 steps") {
            moveSprite(10);
          } else if (action === "Go back 10 steps") {
            moveSprite(-10);
          } else if (action === "Turn 15 degrees") {
            rotateSprite(15);
          } else if (action === "Turn -15 degrees") {
            rotateSprite(-15);
          } else if (action === "Say Hello") {
            sayHello();
          } else if (action === "Disappear") {
            disappear();
          }
        }, index * 1000); // Delay each action for demonstration
      });
    };
  ...
  const moveSprite = (steps) => {
    ...
    setPosition((prevPos) => {
      ...
      updateTransform(newPos, rotation);
      return newPos;
    });
  };
  ...
  const updateTransform = (position, rotation) => {
    const sprite = spriteRef.current;
    sprite.style.transform = `translate(${position.x}px, ${position.y}px) rotate(${rotation}deg)`;
  };
  ...
  const disappear = () => {
    setIsVisible(false);
    setTimeout(() => setIsVisible(true), 2000); // Reappear after 2 seconds
  };

  return (
    <div
      ref={spriteRef}
      className={`w-16 h-16 ${isVisible ? "block" : "hidden"} mt-[30px]`}
